...
- `audio: "browser"` makes a browser tab the WebRTC media peer. It owns the microphone, speaker, and echo cancellation, so the pi host needs no audio devices or native audio packages. pi keeps signaling, the sideband, and delegation. `/live` serves a page on `127.0.0.1:<browserPort>` and prints its URL with a private token. Each `/live` run makes a new token, so old URLs in terminal scrollback or browser history stop working. The page only accepts loopback hosts, same-origin WebSockets, and that token. While it serves, pi also writes the port, URL, pid, and start time to a mode-0600 state file, `${XDG_STATE_HOME:-~/.local/state}/pi-better-openai/live.json`, and removes the file when live mode ends.

For pi on a remote host, forward the port from the machine with the microphone, for example with `LocalForward 8795 127.0.0.1:8795` in `~/.ssh/config` or `ssh -L 8795:127.0.0.1:8795 host`. Open the printed `http://localhost:8795/#…` URL (browsers allow the microphone on `localhost`), click **Enable audio**, and keep the tab open. The tab follows the `/live` run that printed it and stops retrying when live mode ends. Only one pi process per host can serve the page at a time.

On a Mac, [`scripts/live-mac-helper.sh`](scripts/live-mac-helper.sh) does both steps for you. It keeps one SSH ControlMaster connection to the pi host that carries the port forward. Over that connection it reads the state file every 2 seconds and opens the page once for each new `/live` run. The token goes only from the state file to `open`. Run it as `PBO_LIVE_PORT=8795 scripts/live-mac-helper.sh <ssh-host>` (the host must work with `BatchMode=yes`), or install it as a launchd agent from the template [`scripts/live-mac-helper.plist`](scripts/live-mac-helper.plist). Remove any manual `LocalForward` for the same port first; the helper owns the forward.
